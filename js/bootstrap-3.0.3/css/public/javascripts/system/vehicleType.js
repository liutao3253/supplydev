var columns = [[
    ...
    {field:'seatnum', title:'客位数', halign:'center',align:'left', width:100},
    {field:'priceofkm', title:'公里价格', halign:'center',align:'left', width:100},
...
$(function(){
	...
	$('#modify').click(function(){
		...
		$('#e_seatnum').val(row.seatnum);
		$('#e_priceofkm').val(row.priceofkm);
	...
	$('#save').click(function(){	
		...
		var num_priceofkm = null;
		try{
			num_priceofkm = Number($('#e_priceofkm').val());
		}catch(e){}
		if(num_priceofkm == null){
			$('#prompt').html('公里数必须为数字');
			return false;
		}
		
		var re = /^[0-9]+$/;
		if(!re.test($('#e_seatnum').val())){
			$('#prompt').html('座位数必须为整数');
			return false;
		}
		showProgressbar();
		$.ajax({
